...
## 2026-09-13 11:05 北京时间(03:05 UTC) — 实验 G 上线, E 退役
...
| 运行时长 | 约 20 小时 |
| 成交笔数 | **0** |
| 退役原因 | 方向已被证伪: 24h 涨幅 > 25% 的标的未来收益 15m -0.176% / 60m **+0.431%** / 180m **+2.119%** —— 极强动量是**延续**而非反转, 做空期望为负 |

容器已删除, docker-compose 中的服务定义已移除。

### 二、G 上线(B+F 融合)

- 容器 `m3dsc-freqtrade-turbo`, REST :18087, 策略 `M3CarryDipTurbo`, 初始 500 USDT, 最多 5 笔并发。
- 新增: `freqtrade/user_data/strategies/M3CarryDipTurbo.py`, `config/config-turbo-dryrun.json`。
- 入场 = F 的两把钥匙(急跌 15m<=-3.5% 或 1h<=-5%, 且年化费率 <= -15%);
  出场改为**短持快收**: 阶梯 +3.5%/+8%(各减 40%)、最长 6 小时、RSI 修复即走、费率转正即撤。
- 依据: 收益随时间快速衰减, 且 F 的费率收益只占 4.6% —— 不值得为收一次费率长持数小时。

### 三、上线门槛体检(真实数据回放)

最近 24 小时、61 个候选标的逐分钟快照上, G 双门槛命中 **5 个标的 63 次**
(MTL 21 / LSK 15 / SOPH 14 / VTHO 8 / IOST 5), 证明门槛不是空闸。

### 四、脚本与守护同步更新

- `scripts/dshc_compare.py`: D/E 标记为已停用, 新增 G 一行。
- `scripts/dshc_attribution.py`: 新增 G 的 30 分钟归因轮询。
- `scripts/dshc-monitor.sh`: 采集位由 E 改为 G。
