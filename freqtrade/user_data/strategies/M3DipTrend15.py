"""M3-DSH 实验 I: 正费率 + 15 分钟急跌 —— 检验 H 的机制在更短窗口是否同样成立.

与实验 H 的唯一区别: 急跌窗口从 1 小时(-5%)换成 15 分钟(-3.5%)。
出处逻辑、风控内核、名义敞口、费率前提失效处置全部逐字照搬 H, 因此 I vs H 隔离的
就是「急跌窗口长度」这一个变量。

扫描证据(2026-09-15, 事件去重, 前瞻 60 分钟):

    分组                    事件   去极值%   中位%   胜率%   自助 95% 置信区间
    正费率 + 1h急跌 (H)      101   +1.502   +1.261   67.3   [+0.638%, +2.727%]  <- 五关全过
    正费率 + 15m急跌 (I)     100   +0.426   +0.847   59.0   [-0.114%, +1.020%]  <- 下界跨 0

H 的证据强度明显更高, 但 15m 档的事件数几乎相同(100 vs 101), 且去极值仍为正、胜率 59%。
两者并列跑, 可以回答一个有价值的问题:
    「在正费率(上升趋势)里买回调」这个机制, 是只对较深的 1 小时回调有效,
     还是对更浅的 15 分钟回调同样有效?
若 I 也成立 -> 机制稳健, 且信号频率更高(更适合追求持续参与);
若 I 明显弱于 H -> 说明需要「足够深的回调」才值得进场, 15 分钟级别的噪声不足以构成买点。
"""
...
class M3DipTrend15(IStrategy):
    ...
    # ---- 入场: 正费率 + 15 分钟急跌(与 H 的区别只在这里) ----
    DIP_15M_PCT = -3.5          # 15 分钟跌超 3.5%(短促回调)
    ...
    def bot_start(self, **kwargs: Any) -> None:
        ...
        log.info("[I] 正费率+15m急跌策略启动 (%s UTC)",
                 datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S"))
        log.info("[I] 门槛: 15m<=%.1f%% | 年化费率>=%.0f%% | 阶梯止盈 +%.1f%%/+%.1f%% "
                 "| 最长 %d 小时 | 杠杆 %.1fx 风险上限 %.1f%%",
                 self.DIP_15M_PCT, self.MIN_FUNDING_ANN * 100,
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        ok = (dataframe["m3_in_pool"].astype(bool)
              ...
              & (dataframe["volume"] > 0))
        dataframe.loc[ok, ["enter_long", "enter_tag"]] = (1, "dip_trend_15m")
        ...
        if st["n"] % 100 == 0:
            log.info("[I] 评估 %d 次, 信号 %d 次 | 门槛通过率: %s", st["n"], st["sig"],
    ...
    def custom_stake_amount(self, pair: str, current_time: datetime, current_rate: float,
        ...
        try:
            ...
            if not plan.ok:
                log.warning("[I] %s 放弃: %s", pair, plan.reason)
                return 0.0
            ...
            if min_stake and stake < min_stake:
                log.info("[I] %s 跳过: 计算仓位 %.2f 低于交易所最小 %.2f, 不为凑单放大仓位",
            ...
            log.info("[I] %s 仓位: 费率年化%.0f%% 15m%.2f%% 1h%.2f%% 止损距离%.2f%% 杠杆%.1f -> "
            ...
            log.warning("[I] 仓位异常 %s: %s", pair, exc)
            return proposed_stake
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime,
        ...
        if min_stake and amount < min_stake:
            return None
        log.info("[I] %s 阶梯止盈第%d档(浮盈%.2f%% >= %.1f%%): 减仓 %.2f",
    ...
    def confirm_trade_exit(self, pair: str, trade: Trade, order_type: str, amount: float,
        ...
        if d:
            log.info("[I][平仓] %s 原因=%s | 设计风险%.2f%% 实际%.2f%% | 入场 费率%.0f%% 15m%.2f%% "
