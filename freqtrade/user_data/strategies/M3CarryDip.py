...
class M3CarryDip(IStrategy):
    ...
    def custom_stake_amount(self, pair: str, current_time: datetime, current_rate: float,
        ...
        try:
            ...
            # 现在改为: 算出多少就是多少, 达不到交易所最小名义值就放弃这笔(plan.ok=False 已覆盖)。
            if min_stake and stake < min_stake:
                log.info("[F] %s 跳过: 计算仓位 %.2f 低于交易所最小 %.2f, 不为凑单放大仓位",
            ...
            log.info("[F] %s 仓位: 费率年化%.0f%% 15m动量%.2f%% 止损距离%.2f%% 杠杆%.1f -> "
                     ...
                     mom, sd * 100, leverage, stake, stake * leverage, plan.notional_pct * 100,
