"""M3-DSH 实验 H: 正费率 + 1h 急跌 —— 在上升趋势里买回调.

前提翻转的依据(2026-09-15 门槛强度扫描, 详见 docs/STATE.md 第 27 节):
    对 3.2 天、32884 个采样点做 2x2 对照(费率符号 x 是否急跌), 事件去重后:

        分组                          事件   去极值%   中位%   胜率%
        正费率, 不急跌 (beta 对照)     2613   -0.088   -0.056   45.5
        正费率 + 1h急跌                101   +1.502   +1.261   67.3   <- 本策略
        负费率, 不急跌 (beta 对照)      436   -0.210   -0.139   45.0
        负费率 + 1h急跌 (实验 G 现行)    70   +0.002   -0.122   45.7

    全体采样点平均前瞻收益 = 0.000% —— 市场整体是平的, 所以这不是「大盘在涨」的 beta。
    两个 beta 对照组都为负 -> 没有急跌就没有优势, 急跌本身是有效信号。

    「正费率 + 1h急跌」组通过全部三重稳健性检验:
      全样本去极值 +1.502% / 剔掉最大贡献标的后 +1.228% /
      自助重采样 95% 置信区间 [+0.638%, +2.727%] 为正概率 100% / 前后半段 +1.370%, +1.611%
    「负费率 + 1h急跌」(实验 G) 五关全不过: 去极值 +0.002%, 剔掉 LSK 后 -0.754%,
      置信区间 [-1.718%, +1.453%] 为正概率 48.4%, 后半段 -0.440%。

经济解释:
    正费率 = 多头拥挤、处于上升趋势 -> 1 小时跌 5% 是**回调**, 被买回;
    负费率 = 空头拥挤、处于下降趋势 -> 1 小时跌 5% 是**趋势延续**, 继续跌。
    这与项目更早「极强动量是延续而非反转」的结论一致。

与实验 G 的唯一区别就是**费率符号翻转** —— 出场逻辑、阶梯止盈、止损内核、名义敞口风控
全部逐字照搬 G, 这样 G vs H 才能干净地隔离「费率符号」这一个变量。
"""
...
class M3DipTrend(IStrategy):
    ...
    # ---- 入场: 只用通过三重稳健性检验的那个组合 ----
    # 注: 不保留 15 分钟档。扫描显示「正费率 + 15m急跌」去极值 +0.426%、置信区间
    #     [-0.114%, +1.020%] 下界跨 0, 证据强度明显弱于 1h 档, 故不收。
    DIP_1H_PCT = -5.0           # 1 小时跌超 5%(回调)
    MIN_FUNDING_ANN = 0.0       # 年化费率 >= 0(多头拥挤/上升趋势)
    MAX_CHANGE_24H = 60.0
    ...
    # H 的费率方向与 G 相反: 前提是「多头拥挤」。一旦费率转负(空头开始拥挤),
    # 前提就没了 -> 无浮盈即撤。阈值取 -2%(留一点噪声余量)。
    FUNDING_BREAK_ANN = -0.02
    ...
    def bot_start(self, **kwargs: Any) -> None:
        ...
        log.info("[H] 正费率+急跌策略启动 (%s UTC)",
                 datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S"))
        log.info("[H] 门槛: 1h<=%.1f%% | 年化费率>=%.0f%% | 阶梯止盈 +%.1f%%/+%.1f%% "
                 "| 最长 %d 小时 | 杠杆 %.1fx 风险上限 %.1f%%",
                 self.DIP_1H_PCT, self.MIN_FUNDING_ANN * 100,
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        ok = (dataframe["m3_in_pool"].astype(bool)
              & dip1h                                                   # 1 小时急跌(回调)
              & (dataframe["m3_funding_ann"] >= self.MIN_FUNDING_ANN)    # 正费率(多头拥挤)
              ...
              & (dataframe["volume"] > 0))
        dataframe.loc[ok, ["enter_long", "enter_tag"]] = (1, "dip_trend")
        ...
        gates = {
            ...
            "posf": bool(dataframe["m3_funding_ann"].iloc[i] >= self.MIN_FUNDING_ANN),
        ...
        if st["n"] % 100 == 0:
            log.info("[H] 评估 %d 次, 信号 %d 次 | 门槛通过率: %s", st["n"], st["sig"],
    ...
    def custom_stake_amount(self, pair: str, current_time: datetime, current_rate: float,
        ...
        try:
            ...
            # 费率越正(多头越拥挤) -> 回调越可能被买回, 给更多权重(H 的相对 G 是翻转的)
            conf = 1.0
            if m60 <= self.DIP_1H_PCT:
                conf += 0.25               # 已由入场门槛保证, 这里保留结构一致
            if ann >= 0.60:
                conf += 0.15
            ...
            if not plan.ok:
                log.warning("[H] %s 放弃: %s", pair, plan.reason)
                return 0.0
            ...
            if min_stake and stake < min_stake:
                log.info("[H] %s 跳过: 计算仓位 %.2f 低于交易所最小 %.2f, 不为凑单放大仓位",
            ...
            log.info("[H] %s 仓位: 费率年化%.0f%% 15m%.2f%% 1h%.2f%% 止损距离%.2f%% 杠杆%.1f -> "
            ...
            log.warning("[H] 仓位异常 %s: %s", pair, exc)
            return proposed_stake
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime,
        ...
        if min_stake and amount < min_stake:
            return None
        log.info("[H] %s 阶梯止盈第%d档(浮盈%.2f%% >= %.1f%%): 减仓 %.2f",
    ...
    def custom_exit(self, pair: str, trade: Trade, current_time: datetime, current_rate: float,
        ...
        """短持快收: 反弹到位/RSI 修复即走; 前提失效(费率转负)也走; 超时更短."""
        ...
        try:
            ...
            ann, rsi5 = 0.0, 50.0
        if ann <= self.FUNDING_BREAK_ANN and current_profit < 0.01:
            return "funding_flipped"       # 费率转负 -> 「多头拥挤」前提失效
    ...
    def confirm_trade_exit(self, pair: str, trade: Trade, order_type: str, amount: float,
        ...
        if d:
            log.info("[H][平仓] %s 原因=%s | 设计风险%.2f%% 实际%.2f%% | 入场 费率%.0f%% 15m%.2f%% "
