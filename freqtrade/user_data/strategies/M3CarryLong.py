...
class M3CarryLong(IStrategy):
    ...
    RISK_BUDGET = 0.008
    RISK_CEILING = 0.012
    MIN_STAKE_RATIO = 0.03   # 名义口径的单笔下限(交易所最小成交额); 必须 <= NOTIONAL_CAP/max_open_trades
    MAX_STAKE_RATIO = 0.30
    TARGET_STAKE_RATIO = 0.18
    # 2026-09-14 根因修正(与 G/F/B 同款): 单笔权益损失 = 止损距离 x 杠杆。
    # 组合名义敞口按权益比重封顶, 并按同时持仓数做等风险分配。
    NOTIONAL_CAP = 0.30
    ...
    def custom_stake_amount(self, pair: str, current_time: datetime, current_rate: float,
        ...
        try:
            ...
            if min_stake and stake < min_stake:
                log.info("[CARRY] %s 跳过: 计算仓位 %.2f 低于交易所最小 %.2f, 不为凑单放大仓位",
            ...
            log.info("[CARRY] %s 仓位: 费率年化%.0f%% 止损距离%.2f%% 杠杆%.1f -> 保证金%.2f "
                     "(名义%.2f=权益%.1f%%, 止损一发风险%.2f%%) [%s]", pair, ann * 100, sd * 100,
                     leverage, stake, stake * leverage, plan.notional_pct * 100,
